...
import { TodayTasksSheet } from '@/components/shared/TodayTasksSheet'
...
const NAV_ITEMS = [
  ...
  null,
  { href: null,        label: 'To Do',    icon: CheckSquare },
...
export function BottomNav() {
  ...
  return (
    <>
          ...
          {NAV_ITEMS.map((item, i) => {
            ...
            if (!item.href) {
              return (
                ...
                >
                  <Icon size={22} className={cn('transition-colors', tasksOpen ? 'text-[#c4933f]' : 'text-[#555]')} />
                  <span className={cn('text-[10px] font-sans transition-colors', tasksOpen ? 'text-[#c4933f] font-semibold' : 'text-[#555]')}>
                    {item.label}
                  </span>
                </button>
              )
            ...
            return (
              <Link
                ...
                className="flex flex-col items-center gap-0.5 px-3 py-1 min-w-0"
              >
                <Icon size={22} className={cn('transition-colors', active ? 'text-[#c4933f]' : 'text-[#555]')} />
                <span className={cn('text-[10px] font-sans transition-colors', active ? 'text-[#c4933f] font-semibold' : 'text-[#555]')}>
                  {item.label}
                </span>
                {active && <div className="w-1 h-1 rounded-full bg-[#c4933f]" />}
              </Link>
            )
          })}
        </div>
      ...
      <TodayTasksSheet open={tasksOpen} onClose={() => setTasksOpen(false)} />
    </>
  )
}
