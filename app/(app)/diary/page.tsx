      ...
      <div className={`p-4 rounded-2xl neu-card transition-all active:scale-[0.99] ${borderClass ? 'border-l-4 ' + borderClass : ''}`}>
        <div className="flex items-start justify-between mb-1">
          ...
          <div className="flex items-center gap-1.5 ml-2 flex-shrink-0">
            {hasUpcomingReminder && <Bell size={13} className="text-[#c4933f]" />}
            {entry.starred && <Star size={14} className="text-[#c4933f] fill-[#6b82c8]" />}
        ...
        {tags && tags.length > 0 && (
          <div className="flex gap-1 flex-wrap mt-2">
            {tags.slice(0, 3).map(tag => (
              <span key={tag!.id} className="text-xs font-sans text-[#c4933f] bg-[#1e1500] px-2 py-0.5 rounded-full">
                #{tag!.name}
              </span>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            <div className="neu-icon-pit w-16 h-16 rounded-full flex items-center justify-center mb-4">
              <BookOpen size={28} className="text-[#c4933f]" />
            </div>
